...
- 复核单创建时必须选择在检样本（`testing`）和在用方法（`active`）；提交复核时服务端记录样本批次、样本状态和方法版本快照。
- 签发前再次核对：样本不再在检、样本或方法被删除、方法换版（版本号变化）或退役时拦截签发，把被挡原因写入复核单（`blockedReason`）并追加 `block` 审核记录；被拦截的复核单留档不可再提交或签发，补正需新建复核单读取最新状态。
- 复核页展示提交依据快照、样本/方法当前状态核对、过期提示和被挡原因；列表与签发确认均提示拦截后果。
- 请求 ID、结构化日志、全局错误映射和 Redis 分布式限流。
- 提供脱敏运行配置、当前会话、审计汇总和单实体审计历史接口。
- 业务工作台支持查询、新建、状态推进、风险标识及操作审计查看；`ChainBadge`/`ChainTimeline` 复用于批次和样本页，`MethodSelector` 复用于方法页，空结果统一使用 `EmptyState`。
