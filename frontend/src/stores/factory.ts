...
export class EntityStore {
  ...
  async transition(path: string, item: DomainRecord, status: string) { this.patch({ loading: true }); try { await request<DomainRecord>(`/${path}/${item.id}/transition`, { method: 'POST', body: JSON.stringify({ status, expectedVersion: item.version, reason: status === 'signed' ? '复核页签发前核对通过，同意签发' : status === 'peer_review' ? '复核页提交复核并冻结依据' : '前端工作台人工确认' }) }); await this.load(path); } catch (error) { this.patch({ loading: false, error: error instanceof Error ? error.message : String(error) }); throw error; } }
  patchError(error: string) { this.patch({ error }); }
  clearError() { this.patch({ error: '' }); }
  protected patch(value: Partial<EntityState>) { this.subject.next({ ...this.subject.value, ...value }); }
}
